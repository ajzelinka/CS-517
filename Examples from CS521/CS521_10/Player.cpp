...
*/
    ...
    int Player::compareName(Player player) const {
    ...
    string Player::getName() const { return mLast + ", " + mFirst; }

    // Sum all singles, doubles, triples, and home runs
    double Player::getHits() const { return double(mStats[2] + mStats[3] + mStats[4] + mStats[5]); }
    // Divide all hits by at bats
    double Player::getBatting() const { return getHits() / (mStats[1]); }
    // Add walks and hits by pitch to the total hits, and divide by plate appearances
    double Player::getOBP() const { return (getHits() + mStats[6] + mStats[7]) / mStats[0]; }
    // Calculate weighted average, then divide by at bats
    double Player::getSlugging() const {
        double weighted = mStats[2] + (2 * mStats[3]) + (3 * mStats[4]) + (4 * mStats[5]);
        return weighted / mStats[1];
    }

    // Add OBP and slugging average together
    double Player::getOPS() const { return getOBP() + getSlugging(); }

    // Print player name, batting average, and OPS to os
    void Player::print(ostream &os) const {
        os << setw(28) << getName() << " :";
        os << setw(15) << fixed << setprecision(3) << getBatting();
        os << setw(15) << getOPS() << endl;
    }
